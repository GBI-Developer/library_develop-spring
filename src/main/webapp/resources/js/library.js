...
function tableRelatedFunctionCall(table, json, displayInfo) {
  ...
  if(displayInfo == "ユーザ一覧" || displayInfo == "図書一覧"
     || displayInfo == "貸出確認" || displayInfo == PARAM_LENDING_WITHIN_DISPLAY){
    tableRelatedFunction(displayInfo, null);
  }

  // 超過者一覧画面の場合
  if (displayInfo == "超過者一覧") {
...
function tableRelatedFunction(displayInfo, json) {
  ...
  //ユーザ一覧の場合
  if(($("#user").length) && (!$("#book").length) && displayInfo == "ユーザ一覧"){
    table = $("#user");// テーブルDOM
  }
  //図書一覧の場合
  if(($("#book").length) && (!$("#user").length) && displayInfo == "図書一覧"){
    flg = INT_ONE;
    table = $("#book");// テーブルDOM
  }
  //貸出確認画面の場合
  if(($("#book").length) && ($("#user").length) && displayInfo == "貸出確認"){
    flg = INT_TWO;
    table = $("#user");// テーブルDOM
  }
  //貸出中一覧の場合
  ...
  //超過者一覧の場合
  if(($("#user").length) && (displayInfo == "超過者一覧")){
    flg = INT_FOR;
    table = $("#user");// テーブルDOM
  }
 ...
 * @returns:void
 */
function userActiveDisplayChange(displayInfo){
...
/**
 * @概要 チェック数があれば削除確認ボタンを表示.
 * @memberOf: libraryWeb.js.library.js
 * @param:
 * @returns:void
 */
function chkCntIfDeleteBtnDisplay(table) {
...
/**
 * チェックボックス有りの図書一覧を作成する.<br>
 * @memberOf; libraryWeb.js.library.js<br>
 * @param:json 形式の図書情報<br>
 * @returns:table<br>
 */
function displayBooksListHasCheckbox(bookInfoJson){
  var table = $('#book').DataTable({
    stateSave: true,
    data: bookInfoJson,
    columns: [
 ...
 * @returns:void
 */
function pageToggleEvnet() {
  var selecter = '.paginate_button > a';
 ...
 * @returns:void
 */
function lendBookDisabled() {
  var selecter = $('#book tr');
  var str = '"bookLendStatus":"1"';

  selecter.each(function(i, o) {
    var findVal = "";
    if ($(o).find('input[name="id[]"]').val() != null) {
      findVal = $(o).find('input[name="id[]"]').val();
      if (findVal.indexOf(str) > 0){
        $(o).css('opacity','0.8');
        $(o).css('background-color','#EBEBEB');
 ...
 * @returns:void
 */
function bookActiveDisplayChange(displayInfo){
    var thisButton = null; // アクティブボタンセレクタ格納用
    var titleText = ""; //タイトルテキスト
    switch(displayInfo){
    // 図書一覧画面の場合
...
function CheckedLendBookInformation(){
  $('button.lendChange').on('click', function(e){
    ...
    $('input[name="bookIdArray"]').val(list);

    // 貸出確認画面遷移
    window.location.href = "/library/booklendinglist/bookLendConfirm?list="+list+"";
  });
}
...
/**
 * @概要 チェック数があれば貸出確認ボタンを表示.
 * @memberOf: libraryWeb.js.library.js
 * @param:
 * @returns:void
 */
function chkCntIfLendBtnDisplay(table) {
...
function SetParameterModal(data, flg, size) {
  ...
  if(flg == INT_ZERO){
    ...
    $(body).find('td[name="user-registe-date"]').children().val(data.userRegisteDateToString);
  }
  // 図書詳細モーダル
  if(flg == INT_ONE){
    ...
    $(body).find('td[name="book-registe-date"]').children().val(data.bookRegisteDateToString);
  }
  // 貸出確認モーダル
  if(flg == INT_TWO){
    ...
    $(body).find('td[name="user-registe-date"]').children().text(data.userRegisteDateToString);
    // hiddin要素にパラメータ設定
